...
class Groups {
    ...
    static function add($id = 0) {
        ...
        if (!$nameSeo) {
            self::setResponseData('name_seo', l('required_field'));
            return self::$responseData;
        }

        $nameCensured = censured($nameSeo);
        if ($nameCensured != $nameSeo) {
            self::setResponseData('name_seo', l('public_url_contains_invalid_characters'));
            return self::$responseData;
        }

        if (preg_match('/[%#&\'"\/\\\\<>*|]/', $nameSeo)) {
            self::setResponseData('name_seo', l('invalid_public_url'));
            return self::$responseData;
        }

        if (!User::checkNameCompatibilityWithSystem($nameSeo)) {
            self::setResponseData('name_seo', l('public_url_cannot_be_used'));
            return self::$responseData;
        }

        $nameSeo = Router::getNameSeo($nameSeo, $id, 'group', false);
        if (!$nameSeo) {
            self::setResponseData('name_seo', l('public_url_exists'));
            return self::$responseData;
        }
    ...
    static function subscribeRequestAdd($uid, $groupId, $accepted = false, $isSendMail = true) {
        ...
        /*if ($isSendmail
            && Common::isEnabledAutoMail('group_subscribe_request')) {
            $optionSendMail = $user['set_email_mail'];
                ...
                if ($optionSendMail != '2') {
                    ...
                    Common::sendAutomail($user['lang'], $user['mail'], 'group_subscribe_request', $vars);
                }
        }*/
    }
