...
class ListBlocksOrder extends CHtmlBlock {
    ...
    static $listIconSvg = array(
        ...
        'timeline'      => '<svg viewBox="-150 60 950 950">
        ...
        'user_plus'     =>  '<svg viewBox="0 0 1000 1000">
                                <g transform="matrix(1 0 0 -1 0 1536)">
                                    <path d="M346.9,1036c-50.7,0-94,17.9-129.9,53.8s-53.8,79.2-53.8,129.9s17.9,94,53.8,129.9s79.2,53.8,129.9,53.8s94-17.9,129.9-53.8s53.8-79.2,53.8-129.9s-17.9-94-53.8-129.9S397.6,1036,346.9,1036z M806.3,974.8h168.4c4.1,0,7.7-1.5,10.8-4.5s4.5-6.6,4.5-10.8v-91.9c0-4.1-1.5-7.7-4.5-10.8s-6.6-4.5-10.8-4.5H806.3V683.8c0-4.1-1.5-7.7-4.5-10.8s-6.6-4.5-10.8-4.5h-91.9c-4.1,0-7.7,1.5-10.8,4.5s-4.5,6.6-4.5,10.8v168.4H515.3c-4.1,0-7.7,1.5-10.8,4.5s-4.5,6.6-4.5,10.8v91.9c0,4.1,1.5,7.7,4.5,10.8s6.6,4.5,10.8,4.5h168.4v168.4c0,4.1,1.5,7.7,4.5,10.8s6.6,4.5,10.8,4.5h91.9c4.1,0,7.7-1.5,10.8-4.5s4.5-6.6,4.5-10.8V974.8z M454.1,867.6c0-16.6,6.1-30.9,18.2-43.1s26.5-18.2,43.1-18.2h122.5V692.4c-21.7-16-49-23.9-81.8-23.9H137.8c-38.6,0-69.5,11-92.8,33S10,753.8,10,792.4c0,16.9,0.6,33.4,1.7,49.5s3.3,33.5,6.7,52.2s7.6,36,12.7,51.9s12,31.5,20.6,46.7s18.5,28.1,29.7,38.8s24.8,19.2,40.9,25.6s33.9,9.6,53.4,9.6c6.1,0,12.3-2.7,18.7-8.1c25.2-19.5,49.8-34.1,73.9-43.8s50.3-14.6,78.7-14.6s54.6,4.9,78.7,14.6s48.7,24.3,73.9,43.8c6.4,5.4,12.6,8.1,18.7,8.1c42.1,0,76.7-15.3,103.8-45.9H515.3c-16.6,0-30.9-6.1-43.1-18.2c-12.1-12.1-18.2-26.5-18.2-43.1V867.6z"/>
                                </g>
                            </svg>',
    ...
    static function getOrderItemsList($module, $onlyActive = true, $tmpl = null, $admin = false)
    {
        ...
        if ($tmpl == 'edge') {
            ...
            if ($module == 'main_page_block_order') {
                ...
                if (!$admin) {
                    if ($guid != $uidParam && !in_array($p, array('groups_list.php'))) {
                        unset($itemsList['page_add']);
                        unset($itemsList['group_add']);
                    } elseif (in_array($p, array('groups_list.php', 'group_add.php'))){
                        if ($isGroupPage) {
                            unset($itemsList['page_add']);
                        } else {
                            unset($itemsList['group_add']);
                ...
                $itemsList = array(
                    ...
                    /*'menu_calendar_edge' => array(
                                                    'url' => 'calendar',
                                                    'icon' => 'fa-calendar',
                                                    'icon_svg' => 'calendar',
                                                    'tooltip' => l('tooltip_calendar')
                                                    ),
            ...
            } elseif($module == 'member_groups_tabs') {
                $itemsList = array(
                    'menu_inner_wall_posts_edge'  => array('url' => '',
                                                           'url_real'=> Groups::url($groupId, null, null, true, true),
                                                           'tooltip' => l('tooltip_browse_posts')),
        ...
        if($module == 'member_header_menu') {
            if (!$admin && !Moderator::checkAccess(true)) {
                unset($itemsList['menu_moderator_edge']);
            }
    ...
    static function parseMenuItem(&$html, $data, $block, $k, $prf = '', $checkCounter = false, $j = 0)
    {
        ...
        $iconSvg = '';
        $isCleanBlIcon = true;
        if ($block == 'member_header_menu' && isset($data['icon_svg']) && self::getIconSvg($data['icon_svg'])) {
            ...
            if ($isProfileMenuInner && isset($v['count']) && !$v['count'] && !self::isMenuItemProfileTab($k)) {
                ...
                if ($isProfileMenuAddUploadFile) {
                  if ($k == 'menu_inner_videos_edge' || $k == 'menu_inner_photos_edge') {
                        $html->setvar("{$blockItem}_item_upload_file_title", $k == 'menu_inner_photos_edge' ? l('upload_a_new_image') : l('upload_a_new_video'));
                        $html->setvar("{$blockItem}_item_upload_file_type", $k == 'menu_inner_photos_edge' ? 'photo' : 'video');
