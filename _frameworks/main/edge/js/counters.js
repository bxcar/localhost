var CCounters = function() {
    ...
    this.setDataOneCounter = function(key, counter, enabled, nostagger){
        if ($this.counters[key] && $this.counters[key][0]) {
            $this.counters[key].data({key:key, counter:counter,enabled:enabled,nostagger:nostagger})
                               .attr({'data-key':key,'data-counter':counter,'data-enabled':enabled,'data-nostagger':nostagger})
